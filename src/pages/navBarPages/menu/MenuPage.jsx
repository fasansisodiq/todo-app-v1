...
function MenuPage({onMenuOpen}) {
  ...
  return (
    <div onMouseLeave={onMenuOpen}  className=" mt-4 absolute flex flex-col justify-start p-4 pl-5 gap-2 w-80 h-120 shadow-2xl border-4 border-white dark:border-emerald-900 bg-white dark:bg-[#232b25] rounded-2xl lg:text-2xl transition-colors duration-300">
      <div className="flex justify-start items-center lg:gap-5 gap-2 w-full pb-2 lg:pb-4">
        {/* <BackBtn /> */}
        <h1 className="text-center flex items-center justify-center w-full font-semibold text-slate-800 dark:text-yellow-200 ">
          Menu
        </h1>
      </div>
      {menuNavLinkItems.map((item) => (
        ...
        >
          {item.icon}
          <span onClick={onMenuOpen}>{item.element}</span>
        </Link>
      ))}
    </div>
  );
